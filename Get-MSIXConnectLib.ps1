...
Function Compress-MSIXAppInstaller ($Path, $InstallerPath, $InstallerArgument)
{
    ...
    New-LogEntry -LogValue "Multiple files required for app install compressing all content into a self-extracting exe" -Component "Compress-MSIXAppInstaller" -Severity 2
...
Foreach ($Item in $XMLData.MSIXCompressedExport.Items.Directory) {mkdir "$($XMLData.Items.exportpath)\$($Item.RelativePath)"}
Foreach ($Item in $XMLData.MSIXCompressedExport.Items.File) {Copy-Item -Path "$($Item.RelativePath)" -Destination "$($XMLData.Items.exportpath)\$($Item.RelativePath)"}

Invoke-Expression "$($XMLData.Items.exportpath)\$($XMLData.MSIXCompressedExport.Installer.Path) $($XMLData.MSIXCompressedExport.Installer.Arguments)"
'@

    ## Exports the PowerShell script which will be used to restructure the content, and trigger the app install.
    New-LogEntry -LogValue "Set-Content -Value ScriptContent -Path $ScriptFilePath -Force `n`r$ScriptContent" -Component "Compress-MSIXAppInstaller" -Severity 1
    ...
    New-LogEntry -LogValue "Set-Content -Value ScriptContent -Path $($CMDScriptFilePath.Replace($ContainerPath, '')) -Force `n`rPowerShell.exe $ScriptFilePath" -Component "Compress-MSIXAppInstaller" -Severity 1
    Set-Content -Value "PowerShell.exe $ScriptFilePath" -Path $($CMDScriptFilePath.Replace($ContainerPath, '')) -Force
    ...
    ## Including reference to the XML file.
    
#    $FileDetails += "`t`t<File Name=""$($templateFilePath.replace($ContainerPath, ''))"" ParentPath=""$ContainerPath"" RelativePath=""$($templateFilePath.replace($ContainerPath, ''))"" Extension=""xlsx"" />"
...
"@
    ...
    New-LogEntry -LogValue "Set-Content -Value xmlContent -Path $templateFilePath -Force `n`r$xmlContent" -Component "Compress-MSIXAppInstaller" -Severity 1
    ...
    #     If($Item.extension -ne '')
    #         { 
    #             IF($FileIncrement -eq 0)
    #                 { $SEDFiles += "FILE$FileIncrement=""$($Item.Name)"""  }
    #             else 
    #                 { $SEDFiles += "`nFILE$FileIncrement=""$($Item.Name)"""  }
                
    #             $FileIncrement ++
    #         }
    #     Else 
    #         { 
    #             IF($(Get-ChildItem $Item.FullName).count -gt 0)
    #             {
    #                 $DirectoryIncrement ++
    #                 $SEDFolders += "`nSourceFiles$DirectoryIncrement=$($Item.FullName)\"
    #             }
    #         }
...
"@
    ...
    ## Exports the XML file which contains the original file and folder structure.
    New-LogEntry -LogValue "Set-Content -Value xmlContent -Path $SEDOutPath -Force `n`r$SEDExportTemplate" -Component "Compress-MSIXAppInstaller" -Severity 1
    ...
    iexpress.exe /N $SEDOutPath

#    Return $Something... related to installer and silent string.
}
